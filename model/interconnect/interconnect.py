...
# ============================================================================
# Enhanced Interconnect Features
# ============================================================================

class HBM4AddressTranslator:
    """
    HBM4-specific address translation and channel mapping.

    Handles:
    - AXI address to HBM4 channel mapping
    - Stack/channel interleaving
    - Bank/row address extraction
    - Pseudo-channel routing
    """
    ...
    def __init__(
        ...
        # Compute address field widths
        self.stack_bits = max(1, (num_stacks - 1).bit_length())
        self.channel_bits = max(1, (channels_per_stack - 1).bit_length())
        self.pch_bits = max(1, (pseudo_channels - 1).bit_length())
        self.bank_bits = max(1, (banks_per_channel - 1).bit_length())
        self.row_bits = max(1, (rows_per_bank - 1).bit_length())
        self.col_bits = max(1, (cols_per_bank - 1).bit_length())

        # Bit offsets
        self.byte_offset_bits = 3  # 8 bytes per burst
        self.col_lsb = self.byte_offset_bits
        self.bank_lsb = self.col_lsb + self.col_bits
        self.row_lsb = self.bank_lsb + self.row_bits
        self.pch_lsb = self.row_lsb + self.pch_bits
        self.channel_lsb = self.pch_lsb + self.channel_bits
        self.stack_lsb = self.channel_lsb + self.stack_bits

    def translate(
        self,
        axi_addr: int,
        interleave_stack: bool = True,
        interleave_channel: bool = True,
        interleave_bank: bool = False,
    ) -> Dict[str, int]:
        """
        Translate AXI address to HBM4 components.

        Args:
            axi_addr: 64-bit AXI address
            interleave_stack: Enable stack interleaving
            interleave_channel: Enable channel interleaving
            interleave_bank: Enable bank interleaving

        Returns:
            Dictionary with stack, channel, pch, bank, row, col
        """
        # Extract components
        byte_offset = axi_addr & 0x7
        col = (axi_addr >> self.col_lsb) & ((1 << self.col_bits) - 1)
        bank = (axi_addr >> self.bank_lsb) & ((1 << self.row_bits) - 1)
        row = (axi_addr >> self.row_lsb) & ((1 << self.row_bits) - 1)
        pch = (axi_addr >> self.pch_lsb) & ((1 << self.pch_bits) - 1)
        channel = (axi_addr >> self.channel_lsb) & ((1 << self.channel_bits) - 1)
        stack = (axi_addr >> self.stack_lsb) & ((1 << self.stack_bits) - 1)

        # Validate bounds
        channel = channel % self.channels_per_stack
        pch = pch % self.pseudo_channels
        bank = bank % self.banks_per_channel
        stack = stack % self.num_stacks
    ...
    def get_channel_id(self, axi_addr: int) -> int:
        """Get HBM4 channel ID from AXI address"""
        decoded = self.translate(axi_addr)
        return decoded['channel']

    def get_stack_id(self, axi_addr: int) -> int:
        """Get HBM4 stack ID from AXI address"""
        decoded = self.translate(axi_addr)
        return decoded['stack']

    def get_pseudo_channel(self, axi_addr: int) -> int:
        """Get pseudo-channel ID from AXI address"""
        decoded = self.translate(axi_addr)
        return decoded['pseudo_channel']


class InterconnectScheduler:
    """
    Schedules requests through the interconnect based on priority and resources.
    ...
    def __init__(
        ...
        # Queues indexed by priority
        self._queues: Dict[int, deque] = {
            i: deque(maxlen=queue_depth) for i in range(num_queues)
        }

        # Age tracking for starvation prevention
        self._queue_ages: Dict[int, int] = {i: 0 for i in range(num_queues)}
        self._current_age = 0
        ...
        self.stats = {
            ...
            'requests_dequeued': 0,
            'requests_dropped': 0,
            'queues_serviced': {i: 0 for i in range(num_queues)},
        }

    def enqueue(self, request: InterconnectRequest) -> bool:
        """Add request to scheduler queue"""
        if self.enable_priority:
            priority = min(request.qos, self.num_queues - 1)
        else:
            priority = 0
    ...
    def dequeue(self) -> Optional[InterconnectRequest]:
        ...
        # Find the queue to service
        selected_priority = None

        if self.enable_priority and self.enable_starvation_prevention:
            # Age-based priority: prefer older queues, but consider QoS
            best_score = -1
            for p in range(self.num_queues):
                if self._queues[p]:
                    # Score = age factor * QoS weight
                    age_factor = self._current_age - self._queue_ages[p]
                    qos_weight = self.num_queues - p  # Higher QoS = higher weight
                    score = age_factor * 0.3 + qos_weight * 0.7
                    if score > best_score:
                        best_score = score
                        selected_priority = p
        elif self.enable_priority:
            # Pure QoS priority
            for p in range(self.num_queues - 1, -1, -1):
                if self._queues[p]:
                    selected_priority = p
                    break
        else:
            ...
            for i in range(self.num_queues):
                idx = (self._current_age + i) % self.num_queues
                if self._queues[idx]:
                    selected_priority = idx
                    break

        if selected_priority is None:
            return None

        request = self._queues[selected_priority].popleft()
        self._queue_ages[selected_priority] = self._current_age
        self.stats['requests_dequeued'] += 1
        self.stats['queues_serviced'][selected_priority] += 1

        return request

    def get_queue_depth(self, priority: Optional[int] = None) -> int:
        """Get queue depth for specific priority or total"""
    ...
    def reset(self) -> None:
        ...
        for q in self._queues.values():
            q.clear()
        self._queue_ages = {i: 0 for i in range(self.num_queues)}
        self._current_age = 0
        for key in self.stats:
            if key != 'queues_serviced':
                self.stats[key] = 0
            else:
                self.stats[key] = {i: 0 for i in range(self.num_queues)}